...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    int lmic_tx_retVAL = 1;
    tx_returned = False;
    lmic_tx_retVAL = LMIC_setTxData2(1, mydata, sizeof(mydata), 0);
    tx_returned = True
    if (llmic_tx_retVAL == 0){
       Serial.println(F("Packet queued"));
      String do_sendstr = "Packet queued";
...
void loop() {
  if ( !os_queryTimeCriticalJobs(ms2osticksRound((TX_INTERVAL * 1000) - 1000 )) && GOTO_DEEPSLEEP) {
    ...
    Serial.println("Im awake and GOTO_DEEPSLEEP is set to false");
...
//    os_setCallback(&sendjob, do_send);
    // Schedule an uplink now
    //os_setTimedCallback(&sendjob, os_getTime() + sec2osticks(1), do_send);
  }
  if (tx_returned && lmic_ret_VAL != 0){
    os_setCallback(&sendjob, do_send);
