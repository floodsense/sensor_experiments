/*******************************************************************************
   ...
   This code:
      - Uses TT8M1 to completely turn off the sensor using signal from pin 5 
        acheiving 95uAmps current while deepsleep after EV_TXCOMPLETE
      - Further testing needed on SD card support and timing and accuracy of 
...
char filename[15];
int sensorMode = 5;
const unsigned TX_INTERVAL = 120 ;  // Duty cycle
...
static const u1_t PROGMEM APPEUI[8] = { <Enter your key in here> };
void os_getArtEui (u1_t* buf) {
  memcpy_P(buf, APPEUI, 8);
}
// This should also be in little endian format
static const u1_t PROGMEM DEVEUI[8] = { <Enter your key in here> };
void os_getDevEui (u1_t* buf) {
  memcpy_P(buf, DEVEUI, 8);
}
// This key should be in big endian format
static const u1_t PROGMEM APPKEY[16] = { <Enter your key in here> };
...
//  if (!sd.begin(chipSelect, SPI_HALF_SPEED)) Serial.println("SD init Failed."); // sd.initErrorHalt(); // Blocking 
//    cout << pstr("Writing to: ") << name;
//    ofstream sdout(name, ios::out | ios::app);
//    if (!sdout) Serial.println("Open Failed");//error("open failed");
//    sdout << buffchar << endl;
//    // close the stream
//    sdout.close();
//    if (!sdout) Serial.println("Write Failed"); // error("writing failed");
//    cout << endl << "Done" << endl;
//
//    digitalWrite(8, LOW);
//  }
