  ...
  switch (ev) {
    ...
    */
    ...
    case EV_TXCOMPLETE:
      ...
      event_ev = String("About to go to sleep at: ") + String(os_getTime());
      writeToSDCard(event_ev);
      // Sleep now
      sleepnow();
      event_ev = String("Woke up from sleep at: ") + String(os_getTime());
      writeToSDCard(event_ev);
      // Prepare a packet in a relaxed setting
      prepare_packet();         // using sensor mode 5
      delay(10);                // To prevent current spikes or power dips due to LoRa (does it really?)
      event_ev = String("Prepared a packet in realaxed setting");

      // Schedule next transmission right now and also prepares packet for modes 1 2 and 3
      event_ev = String("LoRa: Scheduling a packet");
      writeToSDCard(event_ev);
